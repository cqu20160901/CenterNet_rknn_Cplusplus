...
int detect(char *model_path, char *image_path, char *save_image_path)
{
    ...
    for (int i = 0; i < DetectiontRects.size(); i += 6)
    {
        ...
        char text[256];
        sprintf(text, "%d:%.2f", classId, conf);
        rectangle(src_image, cv::Point(xmin, ymin), cv::Point(xmax, ymax), cv::Scalar(255, 0, 0), 2);
        putText(src_image, text, cv::Point(xmin, ymin + 15), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 255), 2);
    }
...
{
    char model_path[256] = "/home/firefly/zhangqian/rknn/rknpu2_1.4.0/examples/rknn_centernet/model/RK3588/centernet.rknn";
    char image_path[256] = "/home/firefly/zhangqian/rknn/rknpu2_1.4.0/examples/rknn_centernet/test.png";
    char save_image_path[256] = "/home/firefly/zhangqian/rknn/rknpu2_1.4.0/examples/rknn_centernet/test_result.jpg";
